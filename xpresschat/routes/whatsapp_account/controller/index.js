...
import WhatsappAccount from "../model/index.js";
...
export const getAccoundbyId = async (req, res) => {
  try {
    ...
    // const tanent_data= await Tenant.findOne({"admin.adminId":req.user.user_id})
    // console.log(":>>>>>>",tanent_data.id)

    const WhatsappAccount_details = await WhatsappAccount.find({ id: req.params.id })
...
export const getAllAccount = async (req, res) => {
  try {
    ...
    const WhatsappAccount_details = await WhatsappAccount.find({ tenant_id: tanent_data.id })
    if (!WhatsappAccount_details) {
      return res.status(404).json({
        success: false,
        message: "Account not found",
